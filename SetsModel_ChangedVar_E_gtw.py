...
alpha_t = {t: 0.02+0.035*t for t in range(T)}
...
#print(rho_gtw)
#q_g0 = {g : 100 for g in range(G)} #Initial capacity of conventional = 1000 MW, renewable = 2300 MW
q_g0 = {0: 2030, 1: 2460, 2: 2275}

#q_j0 = {j : 50 for j in range(J)}
q_j0 = {0: 4750, 1: 5370, 2: 5020} 

kappa_rw = {w: 0.45 for w in range(W)} #Check for percentage of renewable penetration
kappa_cw = {w: 0 for w in range(W)} #Check for percentage of conventional penetration
Phi = -0
...
def Model(pi_w, tau_tw, beta_tw, c_g, i_g, i_j, alpha_t, rho_gtw, rho_jtw, q_g0, q_j0, kappa_rw, kappa_cw, conjectural_t, P_CO2_max, P_CO2_tw, epsilon_max_g, epsilon_cap_g, xi_gtw, v, beta, G, J, T, W):
    ...
    ## Primary
    M_p1 = 5000         #Reasoning: With a max initial capacity of 5370 MW and the idea of not needing to invest too much extra to fulfill demand
                        #then 10000 should be more than enough to keep the balance.
    ...
    M_p4 = 2500         #Reasoning: Can be quite low as it's the maximum for each timestep and scenario    
    M_p5 = 7000         #Reasoning: As the new capacity shouldn't be too high and the initial max is 5370 then 7000 should be sufficient
    M_p6 = 2500000      #Reasoning: Not really sure but as it could have an impact on the profit then 2500000 should be sufficient
    M_p7 = 200000000    #Reasoning: As the profit should be maximized then we don't really want to make an upper bound, 200000000 should be sufficient
    M_p8 = 2500         #Reasoning: With the max E_g of 6000 and a cap of 5000 then 12000 should be sufficient #only used for conventional generators
    M_p9 = 6000         #Reasoning: With an emission cap of 5000 then 6000 seems fullfilling #only used for conventional generators
    
    ## Dual - Initially just 3 times the primary values besides M_d5 which is times 10 (needed for the dual of investment costs)
    M_d1 = 15000        
...
###-----------------------------------------------------------Introducing Constraints-----------------------------------------------------------###
    ...
    for g in range(G):
        for t in range(T):
            for w in range(W):
                ...
                # m.addConstr(- (beta - Delta_gw[g,w] - 1) * i_g[g] - eta_gtw[g,t,w] * rho_gtw[g,t,w] - phi_g[g] == 0, name="1.5b")

                # m.addConstr((beta - (Delta_gw[g,w] / pi_w[w]) - 1) * pi_w[w] * tau_tw[t,w] * P_CO2_tw[t,w] + mu_gtw[g,t,w] - nu_min_gtw[g,t,w] + nu_max_gtw[g,t,w] == 0, name="1.5c") #E_g[g] changed to E_gtw[g,t,w] on obj   
                # m.addConstr(mu_gtw[g,t,w] - nu_min_gtw[g,t,w] + nu_max_gtw[g,t,w] == 0, name="1.5c") 

                m.addConstr(- (beta - 1 - Delta_gw[g,w]) * (P_CO2_tw[t,w]) + (1 / abs(T)) * mu_gtw[g,t,w] - nu_g_min[g] + nu_g_cap[g] == 0, name="1.5d") #E_g[g] changed to E_gtw[g,t,w] on obj   
                # m.addConstr((beta - 1) * (pi_w[w] * tau_tw[t,w] * P_CO2_tw[t,w] - P_CO2_max) + Delta_gw[g,w] * (P_CO2_tw[t,w] - P_CO2_max) + \
    ...
    for t in range(T):
        for w in range(W):
            m.addConstr((beta - (Delta_gw[g,w] / pi_w[w]) - 1) * pi_w[w] * tau_tw[t,w] * P_CO2_tw[t,w] + gp.quicksum(mu_gtw[g,t,w] - nu_min_gtw[g,t,w] + nu_max_gtw[g,t,w] for g in range(G)) == 0, name="1.5c") #E_g[g] changed to E_gtw[g,t,w] on obj
    ...
    for g in range(G):
        for t in range(T):
            for w in range(W):
                ...
                # m.addConstr(E_gtw[g,t,w] + epsilon_max_g[g] <= M_p2 * (1 - psi_min_gtw[g,t,w]), name="1.6e")
        ...
        for w in range(W):
            ...
            m.addConstr((gamma_gw[g,w] + gp.quicksum(tau_tw[t,w] * (p_tw[t,w] * q_gtw[g,t,w] - c_g[g] * q_gtw[g,t,w] + P_CO2_tw[t,w] * \
                        (E_gtw[g,t,w] - xi_gtw[g,t,w] * q_gtw[g,t,w]))  for t in range(T)) - i_g[g] * q_gbar[g] - E_g[g] * P_CO2_max - sigma_g[g]) >= 0, name="1.6s") #NOT E_g[g] changed to E_gtw[g,t,w]
            m.addConstr((gamma_gw[g,w] + gp.quicksum(tau_tw[t,w] * (p_tw[t,w] * q_gtw[g,t,w] - c_g[g] * q_gtw[g,t,w] + P_CO2_tw[t,w] * \
                        (E_gtw[g,t,w] - xi_gtw[g,t,w] * q_gtw[g,t,w]))  for t in range(T)) - i_g[g] * q_gbar[g] - E_g[g] * P_CO2_max - sigma_g[g]) <= M_p7 * (1 - psi_fA_gtw[g,t,w]), name="1.6t") #NOT E_g[g] changed to E_gtw[g,t,w]
    ...
    for j in range(J):
        for t in range(T):
            for w in range(W):
                ...
                # m.addConstr(- (beta - Delta_jw[j,w] - 1) * i_j[j] - eta_jtw[j,t,w] * rho_jtw[j,t,w] - phi_j[j] == 0, name="1.10b")
    ...
    for j in range(J):
        ...
        for w in range(W):
            ...
            # Above constraints are problematic, as the need for the generators to make a profit for them to work    

    ### Objective function    
    objective = gp.quicksum(pi_w[w] * tau_tw[t,w] * (beta_tw[t, w] * d_tw[t, w] - (1/2) * alpha_t[t] * d_tw[t, w] * d_tw[t, w] + \
    gp.quicksum(P_CO2_tw[t,w] * (E_gtw[g,t,w] - xi_gtw[g,t,w] * q_gtw[g,t,w]) - c_g[g] * q_gtw[g,t,w] for g in range(G))) for w in range(W) for t in range(T)) - \
    ...
    if m.Status == GRB.OPTIMAL:
        ...
        Con_CO2_cost = gp.quicksum(pi_w[w] * tau_tw[t,w] * (gp.quicksum(P_CO2_tw[t,w] * (E_gtw[g,t,w].X - xi_gtw[g,t,w] * q_gtw[g,t,w].X) - c_g[g] * q_gtw[g,t,w].X for g in range(G))) for t in range(T) for w in range(W))
