...
W = 3 #Number of Scenarios
G = 2 #Number of Generators
GC = [1,0] #Generator placement (1 for conventional, 0 for renewable)
GR = [0,1] #Generator placement (0 for conventional, 1 for renewable)
RCP = [1,1,1,0.6,0.6] #Capacity factors for the generators, thought it could be used as a range
total_time = 8760  # Total sum required for each scenario
time_lb = 3000
time_ub = 6000
...
Phi = 0
conjectural_t = {i: -alpha_t[i]*(1+Phi) for i in range(T)} #Not sure if these parameters are actually parameters
e_gCap = [10, 10] #Emission cap for generator [ton]
P_CO2=55 #Price of CO2 per ton [Euros]
Zeta_g = [0.5, 0] #Intensity factor for generator [ton/MWh]

#beta = 1 #Not sure if these parameters are actually parameters
#v_g = {i: 1/G for i in range(G)}

def MIQP_RES_Emis(pi_w, tau_tw, beta_tw, alpha_t, c_g, i_g, rho_gtw, q_g0, kappa, conjectural_t, T, W, G, GR, GC, e_gCap, P_CO2, Zeta_g):
    ...
    objective = gp.quicksum(
    pi_w[w] * tau_tw[t, w] * (beta_tw[t, w] * d_tw[t, w] - (1/2) * alpha_t[t] * d_tw[t, w] ** 2 - gp.quicksum(c_g[g] * q_gtw[g, t, w] for g in range(G)) 
     - gp.quicksum(P_CO2 * (eps_gtw[g,t,w] - e_gCap[g]) for g in GC)) for w in range(W) for t in range(T)) - gp.quicksum(i_g[g] * q_gbar[g] for g in range(G))
...
MIQP_RES_Emis(pi_w, tau_tw, beta_tw, alpha_t, c_g, i_g, rho_gtw, q_g0, kappa, conjectural_t, T, W, G, GR, GC, e_gCap, P_CO2, Zeta_g)
